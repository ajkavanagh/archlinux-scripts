...
DRACUT_CMD_LINE_CONF="${DRACUT_CONF_DIR}/cmdline.conf"
DRACUT_FLAGS_CONF="${DRACUT_CONF_DIR}/flags.conf"
DRACUT_INSTALL_ITEMS_CONF="${DRACUT_CONF_DIR}/install_items.conf"
...
root_crypt_p3_UUID=$(echo "$block_to_uuid" | awk -v pat="${ROOT_CRYPT_NAME}" '$0~pat{ print $2 }')
swap_crypt_p2_UUID=$(echo "$block_to_uuid" | awk -v pat="${SWAP_CRYPT_NAME}" '$0~pat{ print $2 }')

# CMDLINE for dracut
cmd_line=""
#cmd_line="$cmd_line rd.luks.uuid=luks-${swap_UUID}"
#cmd_line="$cmd_line rd.luks.uuid=luks-${root_UUID}"
#cmd_line="$cmd_line rd.luks.uuid=luks-${home_UUID}"
#cmd_line="$cmd_line rd.luks.timeout=0"
cmd_line="$cmd_line root=/dev/mapper/${ROOT_CRYPT_NAME}"
#cmd_line="$cmd_line root=/dev/disk/by-uuid/${root_crypt_p3_UUID}"
cmd_line="$cmd_line rootflags=subvol=@"
cmd_line="$cmd_line rootfstype=btrfs"
cmd_line="$cmd_line loglevel=3 resume=/dev/mapper/${SWAP_CRYPT_NAME}"
#cmd_line="$cmd_line rw bgrt_disable"
cmd_line="$cmd_line rw noatime"
cmd_line="kernel_cmdline=\"$cmd_line\""

echo $cmd_line > ${DRACUT_CMD_LINE_CONF}

# Set up the dracut flags
cat << EOF > ${DRACUT_FLAGS_CONF}
compress="zstd"
hostonly="no"
EOF

# Set up additional install items
cat << EOF > ${DRACUT_INSTALL_ITEMS_CONF}
...
# NOTE: do not use options 'swap' for swap file as it will format the swap partition on boot
CRYTPTTAB=/mnt/etc/crypttab
cat << EOF > $CRYTPTTAB
# <target name>	<source device>		<key file>	<options>
${ROOT_CRYPT_NAME}	UUID=${root_UUID}	none	luks
${SWAP_CRYPT_NAME}	UUID=${swap_UUID}	none	luks
${HOME_CRYPT_NAME}	UUID=${home_UUID}	none	luks
EOF
...
# install the systemd-boot hook to update bootctl anytime it is updated.
mkdir -p "/mnt/etc/pacman.d/hooks"
cat << EOF > "/mnt/etc/pacman.d/hooks/998-systemd-boot.hook"
[Trigger]
Type = Package
Operation = Install
Operation = Upgrade
...
Exec = /usr/bin/bootctl update --esp-path=/boot
EOF

# run dracut to create the images.
linux_version=$(arch-chroot /mnt pacman -Ql linux | grep vmlinuz | cut -d '/' -f 5)
linux_lts_version=$(arch-chroot /mnt pacman -Ql linux-lts | grep vmlinuz | cut -d '/' -f 5)

arch-chroot /mnt /usr/bin/dracut --kver "${linux_version}" --uefi --force
arch-chroot /mnt /usr/bin/dracut --kver "${linux_lts_version}" --uefi --force
